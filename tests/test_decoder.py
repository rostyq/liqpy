...
def test_decode_status_example(decoder, example):
    ...
    assert o.get("amount") == Decimal(example["amount"])
    assert o.get("sender_commission") == Decimal(example["sender_commission"])
    assert o.get("agent_commission") == Decimal(example["agent_commission"])
    assert o.get("amount_debit") == Decimal(example["amount_debit"])
    assert o.get("amount_credit") == Decimal(example["amount_credit"])
    assert o.get("commission_debit") == Decimal(example["commission_debit"])
    assert o.get("commission_credit") == Decimal(example["commission_credit"])
    assert o.get("sender_bonus") == Decimal(example["sender_bonus"])
    assert o.get("amount_bonus") == Decimal(example["amount_bonus"])
