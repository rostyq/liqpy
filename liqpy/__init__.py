...
NOW = ContextVar[datetime | None]("liqpy-now", default=None)

DateTuple = tuple[int, int, int]  # (year, month, day)
DateType = datetime | date | str | float | int | DateTuple | timedelta


def _get_now() -> datetime:
    return NOW.get() or datetime.now(UTC)
...
def _(value: str, /):
    if value.startswith(("P", "-P")):
        return _get_now() + parse_isoduration(value)
    else:
