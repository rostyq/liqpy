...
from liqpy.api import DATE_FMT
...
CURRENCY_TYPE = CategoricalDtype(categories=[item.value for item in Currency])
ACTION_TYPE = CategoricalDtype(categories=[item.value for item in Action])

STATUS_TYPE = CategoricalDtype(categories=[item.value for item in Status])
CODE_TYPE = CategoricalDtype(categories=[item.value for item in Code])
PAYWAY_TYPE = CategoricalDtype(categories=[item.value for item in PayWay])

COLUMN_TYPES = defaultdict(
    lambda: STRING_TYPE,
    {
        "ID": ID_TYPE,
        "AMOUNT": FLOAT_TYPE,
